    ...
    for _ in range(50):
        ...
        while len(new)<200:
            ...
            if random.random()<0.3:
                lst=list(child); i=random.randrange(6)
                lst[i]=np.random.choice(range(1,46),p=pf/pf.sum()); child=tuple(sorted(set(lst))[:6])
            new.append(child)
        pop=new
    final=[]
...
# 4) UI
st.header("▶ Backtest 1151→max")
res=[]
for d in range(1151,df["회차"].max()+1):
    s=predict_draw(d); a=set(df[df["회차"]==d][nums].iloc[0])
    res.append({"회차":d,"max_hits":max(len(set(x)&a) for x in s)})
bt=pd.DataFrame(res)
st.write("Avg max hits:",bt["max_hits"].mean(), "3+-rate:",(bt["max_hits"]>=3).mean())

st.header("▶ Next Draw")
nd=df["회차"].max()+1; preds=predict_draw(nd)
st.write(f"{nd}회차 예측 10세트"); st.table(pd.DataFrame({"Set":range(1,11),"Combo":preds}))
